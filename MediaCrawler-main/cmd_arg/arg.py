...
async def parse_cmd(argv: Optional[Sequence[str]] = None):
    ...
    def main(
        ...
        task_id: Annotated[
            ...
            typer.Option(
                "--task_id",
                help="Associated task ID for data attribution",
                rich_help_panel="Basic Configuration",
            ),
        ] = "",
        publish_time_type: Annotated[
            ...
            typer.Option(
                "--publish_time_type",
                help="Publish time filter for search: 0=unlimited, 1=one day, 7=one week, 180=six months",
                rich_help_panel="Basic Configuration",
            ),
        ] = config.PUBLISH_TIME_TYPE if hasattr(config, 'PUBLISH_TIME_TYPE') else 0,
    ) -> SimpleNamespace:
        """MediaCrawler 命令行入口"""
        ...
        # 设置发布时间过滤（仅抖音平台生效）
        if hasattr(config, 'PUBLISH_TIME_TYPE'):
