...
class CDPBrowserManager:
    ...
    def __init__(self, user_data_dir_override: Optional[str] = None):
        self.launcher = BrowserLauncher()
        self.browser: Optional[Browser] = None
        self.browser_context: Optional[BrowserContext] = None
        self.debug_port: Optional[int] = None
        self._cleanup_registered = False
        self.user_data_dir_override = user_data_dir_override
    ...
    async def _launch_browser(self, browser_path: str, headless: bool):
        ...
        """
        # 确保 Xvfb 虚拟显示器运行（Linux 无头服务器必须）
        self._ensure_xvfb()

        # Set user data directory (if save login state is enabled)
        user_data_dir = None
        if self.user_data_dir_override:
            user_data_dir = self.user_data_dir_override
    ...
    def _cleanup_stale_xvfb(self):
        ...
        try:
            ...
            for pid, display in xvfb_to_kill:
                try:
                    ...
                    if os.path.exists(lock_file):
                        os.remove(lock_file)
                    utils.logger.info(f"[CDPBrowserManager] Cleaned up stale Xvfb PID={pid} display={display}")
                except (ProcessLookupError, FileNotFoundError):
                    pass
                except Exception as e:
                    utils.logger.warning(f"[CDPBrowserManager] Failed to kill Xvfb PID={pid}: {e}")
                                ...
                                for env_var in env_data.split("\0"):
                                    if env_var.startswith("DISPLAY="):
                                        ...
                                        if chrome_display in killed_displays:
                                            ...
                                            utils.logger.info(f"[CDPBrowserManager] Cleaned up orphan Chrome PID={chrome_pid}")
                                        break
            ...
            if xvfb_to_kill:
                utils.logger.info(f"[CDPBrowserManager] Cleaned up {len(xvfb_to_kill)} stale Xvfb processes")
        except Exception as e:
            utils.logger.warning(f"[CDPBrowserManager] Error during stale Xvfb cleanup: {e}")
    ...
    def _ensure_xvfb(self):
        ...
        # 如果 DISPLAY 已经设置且有效，跳过
        current_display = os.environ.get("DISPLAY", "")
        if current_display and self._check_display(current_display):
            utils.logger.info(f"[CDPBrowserManager] DISPLAY={current_display} already set and working")
            return
                ...
                for line in result.stdout.strip().split("\n"):
                    ...
                    for part in parts:
                        if part.startswith(":"):
                            display = part
                            break
                    if display and self._check_display(display):
                        os.environ["DISPLAY"] = display
                        utils.logger.info(f"[CDPBrowserManager] Using existing Xvfb display {display}")
                        return
        except Exception:
            pass
        ...
        for display_num in range(99, 120):
            ...
            if self._check_display(display):
                os.environ["DISPLAY"] = display
                utils.logger.info(f"[CDPBrowserManager] Using Xvfb display {display}")
                return

            # 检查这个 display 编号是否被占用
            lock_file = f"/tmp/.X{display_num}-lock"
            if os.path.exists(lock_file):
                continue
            ...
            try:
                ...
                if self._check_display(display):
                    os.environ["DISPLAY"] = display
                    utils.logger.info(f"[CDPBrowserManager] Started Xvfb on display {display}")
                    return
            except Exception as e:
                utils.logger.warning(f"[CDPBrowserManager] Failed to start Xvfb on {display}: {e}")
                continue

        utils.logger.warning("[CDPBrowserManager] Could not start Xvfb, browser may fail to launch")
    ...
    async def _get_browser_websocket_url(self, debug_port: int) -> str:
        ...
        # 调试: 检查浏览器进程状态
        if self.launcher and self.launcher.browser_process:
            poll = self.launcher.browser_process.poll()
            utils.logger.info(f"[CDPBrowserManager] Before WebSocket URL fetch: browser poll={poll}")
        try:
            ...
            import traceback
            utils.logger.error(f"[CDPBrowserManager] Failed to get WebSocket URL: {type(e).__name__}: {e}")
            utils.logger.error(f"[CDPBrowserManager] Traceback: {traceback.format_exc()}")
            # 检查浏览器进程状态
            if self.launcher and self.launcher.browser_process:
                poll = self.launcher.browser_process.poll()
                utils.logger.error(f"[CDPBrowserManager] After WebSocket URL fetch failed: browser poll={poll}")
            raise
    ...
    async def _connect_via_cdp(self, playwright: Playwright):
        ...
        try:
            if config.CDP_CONNECT_EXISTING:
                ...
                # Chrome may return ws://0.0.0.0 or ws://localhost, replace with 127.0.0.1
                # to avoid IPv6 resolution issues
                ws_url = ws_url.replace("ws://0.0.0.0:", "ws://127.0.0.1:").replace("ws://localhost:", "ws://127.0.0.1:")
