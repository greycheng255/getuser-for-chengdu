...
import {
  ReloadOutlined, FireOutlined, LinkOutlined, GlobalOutlined,
...
const HotpointListPanel: React.FC<Props> = ({ compact = false }) => {
        ...
        actions={[
          item.hot ? (
            <Tooltip key="hot" title="热度">
              <span style={{ color: '#FF4D4F' }}><FireOutlined /> {item.hot}</span>
            </Tooltip>
          ) : null,
          <Tooltip key="open" title="在新窗口打开">
            <a href={item.url} target="_blank" rel="noopener noreferrer"><LinkOutlined /> 打开</a>
          </Tooltip>,
        ].filter(Boolean)}
      >
        <List.Item.Meta
          avatar={
            <div style={{ width: 32, height: 32, borderRadius: '50%', backgroundColor: rankColor,
              color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontWeight: 'bold', fontSize: 14 }}>{item.rank}</div>
          }
          ...
          description={
            <Space size="small" wrap>
              ...
              {item.extra?.comments !== undefined ? <Tag>评论 {String(item.extra.comments)}</Tag> : null}
  ...
  return (
    <div style={compact ? {} : { padding: 0 }}>
      <Card size="small" style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          <Row gutter={24} style={{ flex: 1 }}>
            <Col><Statistic title="国内平台" value={chinaPlatforms.length} suffix="个" /></Col>
            <Col><Statistic title="国外平台" value={globalPlatforms.length} suffix="个" /></Col>
            <Col><Statistic title="热点总数" value={totalItems} suffix="条" /></Col>
          </Row>
          <Space>
            <Input placeholder="搜索热点标题" prefix={<SearchOutlined />} value={searchKeyword}
              onChange={e => setSearchKeyword(e.target.value)} allowClear style={{ width: 200 }} />
            <Button icon={<ReloadOutlined />} onClick={handleRefresh} loading={loading}>刷新缓存</Button>
            <Button type="primary" onClick={handleRefreshAll} loading={loading}>强制重新采集</Button>
          </Space>
        </div>
      </Card>
          ...
          items={[
            {
              ...
              children: (
                <Tabs tabPosition="left" activeKey={activePlatform} onChange={setActivePlatform}
                  items={buildPlatformTabs('china')} style={{ minHeight: 400 }} />
              ),
            },
            {
              key: 'global',
              ...
              children: (
                <Tabs tabPosition="left" activeKey={activePlatform} onChange={setActivePlatform}
                  items={buildPlatformTabs('global')} style={{ minHeight: 400 }} />
              ),
            },
          ]}
        />
...
export default HotpointListPanel;
