...
class XhsDbStoreImplement(AbstractStore):
        ...
        async with get_session() as session:
            ...
            try:
                ...
                await check_and_save_lead(
                    ...
                    data_id_field="note_id"
                )
            except Exception as e:
                utils.logger.warning(f"[XhsDbStoreImplement.store_content] Lead detection error: {e}")
    ...
    async def add_content(self, session: AsyncSession, content_item: Dict):
        ...
        task_id = task_id_var.get() if hasattr(task_id_var, 'get') else ""
        note = XhsNote(
            task_id=task_id if task_id else content_item.get("task_id", ""),
    ...
    async def store_comment(self, comment_item: Dict):
        ...
        async with get_session() as session:
            ...
            try:
                ...
                if not task_id:
                    task_id = "manual_run"
                # 查询评论所属笔记的标题，用于关键词相关性判断
                note_title = ""
                note_id = comment_item.get("note_id")
                if note_id:
                    from store.xhs.models import XhsNote
                    note_result = await session.execute(
                        select(XhsNote.title).where(XhsNote.note_id == note_id)
                    )
                    note_row = note_result.fetchone()
                    if note_row and note_row[0]:
                        note_title = note_row[0]
                await check_and_save_lead(
                    task_id=task_id,
                    platform="xhs",
                    data_type="comment",
                    ...
                    extra_title=note_title
                )
            except Exception as e:
                utils.logger.warning(f"[XhsDbStoreImplement.store_comment] Lead detection error: {e}")
    ...
    async def add_comment(self, session: AsyncSession, comment_item: Dict):
        ...
        comment = XhsNoteComment(
            task_id=task_id if task_id else comment_item.get("task_id", ""),
...
class XhsExcelStoreImplement(ExcelStoreBase):
    ...
    def __init__(self, *args, **kwargs):
        # Initialisation is performed once by ExcelStoreBase.get_instance().
        pass
