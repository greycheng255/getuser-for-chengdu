...
图像生成服务
使用多种方式生成图片：
1. onellm (doubao-seedream) - 首选，中文场景表现优秀
2. AI Agent API (lk888.ai) - 备用，gpt-image-2
3. LLM 先把内容总结成精准中文 prompt（提升图文相关性）
4. Pollinations.ai 免费 API - 兜底
5. OpenAI DALL-E - 兜底

设计原则：
- 图片必须与文字内容强相关（用 LLM 生成精准中文场景描述）
- 失败时多模型重试，不再使用本地渐变图（避免发布与内容无关的垃圾图）
- 所有图片为竖图 3:4，适配小红书
"""
...
class ImageGenerationService:
    ...
    def __init__(self):
        ...
        self.api_base = os.getenv('OPENAI_BASE_URL') or os.getenv('AI_URL', 'https://api.openai.com/v1')
        self.ai_service = ai_service
        # onellm 配置
        self.onellm_api_key = os.getenv('ONELLM_API_KEY')
        self.onellm_base_url = os.getenv('ONELLM_BASE_URL', 'http://212.129.240.112:4000').rstrip('/')

    # ==================== LLM 总结 prompt ====================

    def _summarize_to_image_prompt(self, title: str, content: str,
                                    keywords: List[str] = None,
                                    brand_name: str = None,
                                    scene_role: str = 'cover') -> Optional[str]:
        """
...
6. 不要输出任何解释性文字，只输出 prompt 本身"""
            ...
            # 优先使用 AGENT_API（.env 中的 OPENAI_BASE_URL/OPENAI_API_KEY），fallback 到 lk888
            agent_base = os.getenv('OPENAI_BASE_URL', '').rstrip('/')
            agent_key = os.getenv('OPENAI_API_KEY') or os.getenv('AGENT_API_KEY')

            if agent_base and agent_key:
                # 使用 AGENT_API（212.129.240.112:31003）
                llm_url = f'{agent_base}/chat/completions'
                llm_key = agent_key
                llm_model = 'gpt-4o-mini'  # AGENT_API 通用模型
                logger.info(f"[LLM Prompt] 使用 AGENT_API: {llm_url}")
            else:
                # Fallback: 使用 lk888.ai
                llm_url = f'{self.ai_service.base_url}/v1/chat/completions'
                llm_key = self.ai_service.api_key
                llm_model = 'gpt-5.4'
                logger.info(f"[LLM Prompt] 使用 lk888.ai: {llm_url}")
    ...
    def _build_fallback_prompt(self, title: str, content: str,
                                keywords: List[str] = None,
    ...
    def generate_image(self, prompt: str, size: str = "1024x1792", quality: str = "hd") -> Optional[str]:
        """
        生成图片 - 多模型重试链
        ...
        for model in self.ONELLM_MODELS:
            result = self._generate_with_onellm(prompt, size, model)
            if result:
                return result

        # 2. 备：lk888.ai gpt-image-2
        for model in self.IMAGE_MODELS:
            result = self._generate_with_ai_agent(prompt, size, model)
            if result:
                return result

        # 3. 备：Pollinations.ai
        result = self._generate_with_pollinations(prompt, size)
        if result:
            return result

        # 4. 备：OpenAI DALL-E（如果配置了）
        if self.api_key:
            result = self._generate_with_openai(prompt, size, quality)
            if result:
                return result
    ...
    def _generate_with_onellm(self, prompt: str, size: str = "1024x1792",
                                model: str = 'ai6700/doubao-seedream-4-5-251128') -> Optional[str]:
        """
        使用 onellm 多媒体生成 API 生成图片

        文档：onellm_skill_client.md
        ...
        try:
            ...
            try:
                ...
                resp = requests.post(
                    sync_url, headers=headers, json=payload, timeout=180,
                )
                if resp.status_code == 200:
                    data = resp.json()
            ...
            resp = requests.post(
                async_url, headers=headers, json=payload, timeout=30,
            )
            if resp.status_code != 200:
            ...
            ).get('task_id') if isinstance(task_data.get('data'), dict) else None
            ...
            for attempt in range(max_attempts):
                ...
                status_str = sdata.get('status') or (
                    sdata.get('data', {}) or {}
                ).get('status', '') if isinstance(sdata.get('data'), dict) else ''
    ...
    def _extract_onellm_image_url(self, data: Dict) -> Optional[str]:
        ...
        # data.images[0].url
        images = data.get('images') or (data_field or {}).get('images') if isinstance(data_field, dict) else data.get('images')
    ...
    def _generate_with_ai_agent(self, prompt: str, size: str = "1024x1792",
                                 model: str = 'gpt-image-2') -> Optional[str]:
        ...
        try:
            ...
            result = self.ai_service.submit_media_task(
                model=model,
                prompt=prompt,
            ...
            task_id = (result.get('data') or {}).get('task_id') if isinstance(result.get('data'), dict) else None
            if not task_id:
                # 部分接口直接返回 image_url
            ...
            for attempt in range(max_attempts):
                time.sleep(2)
                status = self.ai_service.get_task_status(str(task_id))
    ...
    def _generate_with_openai(self, prompt: str, size: str, quality: str) -> Optional[str]:
    ...
    def _generate_with_pollinations(self, prompt: str, size: str = "1024x1792") -> Optional[str]:
    ...
    def generate_xiaohongshu_cover(self, title: str, content: str,
                                   keywords: List[str] = None,
                                   brand_name: str = None) -> Optional[str]:
        """
        生成小红书封面图 - 用 LLM 总结精准中文 prompt
        """
        prompt = self._summarize_to_image_prompt(
        ...
        return self.generate_image(prompt, size="1024x1792", quality="hd")

    def generate_xiaohongshu_images(self, title: str, content: str = "",
                                     keywords: List[str] = None,
                                     count: int = 3,
                                     brand_name: str = None) -> List[str]:
        """
        ...
        # 第 1 张：封面图
        cover_prompt = self._summarize_to_image_prompt(
            title=title, content=content, keywords=keywords,
            brand_name=brand, scene_role='cover'
        ) or self._build_fallback_prompt(title, content, keywords, brand)

        cover_image = self.generate_image(cover_prompt, size="1024x1792", quality="hd")
        if cover_image:
        ...
        for i in range(count - 1):
            ...
            detail_prompt = self._summarize_to_image_prompt(
            ...
            img = self.generate_image(detail_prompt, size="1024x1792", quality="hd")
            if img:
                images.append(img)
    ...
    def save_base64_to_local(self, base64_image: str,
                              brand_name: str = None,
                              task_id: int = None,
                              index: int = 0,
                              subdir: str = 'xiaohongshu') -> Optional[str]:
        """
        ...
        try:
            ...
            # 确定 project_root：优先用 backend 目录的父级
