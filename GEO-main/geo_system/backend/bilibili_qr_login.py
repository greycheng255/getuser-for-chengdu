    ...
    with _loop_lock:
        ...
        def _run_loop():
            ...
            try:
                ...
                logger.error(f"[BiliQR] 全局事件循环异常: {e}")

        t = threading.Thread(target=_run_loop, daemon=True, name="bili-qr-loop")
        t.start()
        logger.info("[BiliQR] 全局事件循环已启动")
        return _loop
...
class BilibiliQRLogin:
    ...
    async def init_browser(self):
        try:
            ...
            logger.error(f"[BiliQR] 初始化浏览器失败: {e}")
            return False

    async def start_qr_login(self, session_id: str) -> Dict:
        try:
            logger.info(f"[BiliQR] 开始二维码登录，会话ID: {session_id}")
            ...
            # 访问B站登录页
            logger.info("[BiliQR] 访问B站登录页面...")
            await page.goto('https://passport.bilibili.com/login', timeout=30000, wait_until='domcontentloaded')
            ...
            for selector in [
                ...
                try:
                    ...
                    if await el.count() > 0:
                        ...
                        logger.info(f"[BiliQR] 点击扫码登录 tab: {selector}")
                        break
                except Exception:
                    continue
            ...
            for selector in [
                ...
                try:
                    ...
                    if await el.count() > 0:
                        ...
                        if qr_src:
                            logger.info(f"[BiliQR] 获取到二维码，selector={selector}")
                            break
                except Exception:
                    continue
            ...
            if not qr_src:
                try:
                    qr_container = page.locator('.login-scan-box, .qrcode, .login-scan').first
                    if await qr_container.count() > 0:
                        ...
                        logger.info("[BiliQR] 使用截图作为二维码")
                except Exception as e:
                    logger.error(f"[BiliQR] 截图失败: {e}")
            ...
            logger.info(f"[BiliQR] 后台任务已调度，session_id={session_id}")
            ...
            logger.error(f"[BiliQR] 启动登录失败: {e}")
    ...
    async def _check_login_status(self, session_id: str):
        ...
        try:
            while waited < timeout:
                ...
                try:
                    ...
                    if await self._detect_verification_page(page):
                        ...
                        logger.info(f"[BiliQR] 会话 {session_id} 进入验证码阶段")
                        return

                    # 登录成功：URL 跳转到 bilibili.com
                    if 'passport.bilibili.com/login' not in current_url and 'bilibili.com' in current_url:
                        logger.info(f"[BiliQR] 会话 {session_id} 登录成功，URL: {current_url}")
                        break
                    ...
                    logger.error(f"[BiliQR] 检测状态出错: {e}")
                    await asyncio.sleep(2)
                    waited += 2

            current_url = page.url or ''
            if 'passport.bilibili.com/login' not in current_url and 'bilibili.com' in current_url:
                logger.info(f"[BiliQR] 处理登录成功流程，URL: {current_url}")
                ...
                try:
                    ...
                    logger.warning(f"[BiliQR] 访问B站首页失败（继续）: {e}")

                # 访问创作中心
                try:
                    await page.goto('https://member.bilibili.com/platform/upload-manager/article', timeout=15000, wait_until='domcontentloaded')
                    await asyncio.sleep(2)
                except Exception:
                    pass

                cookies = await context.cookies()
                logger.info(f"[BiliQR] 获取到 {len(cookies)} 个 Cookie")

                key_cookies = ['SESSDATA', 'bili_jct', 'DedeUserID', 'sid']
                found_keys = [c.get('name') for c in cookies if c.get('name') in key_cookies]
                logger.info(f"[BiliQR] 关键 Cookie: {found_keys}")

                has_sessdata = any(c.get('name') == 'SESSDATA' for c in cookies)
                logger.info(f"[BiliQR] SESSDATA 认证 Token: {'已获取' if has_sessdata else '未获取'}")
                ...
                try:
                    ...
                    if state_path:
                        ...
                        logger.info(f"[BiliQR] storage_state 已保存到 {state_path}")
                except Exception as e:
                    logger.error(f"[BiliQR] 保存 storage_state 失败: {e}")
            ...
            logger.info(f"[BiliQR] 会话 {session_id} 扫码超时")
            session['status'] = 'timeout'
            await self._close_session(session_id)

        except Exception as e:
            logger.error(f"[BiliQR] 检测登录状态失败: {e}")
        ...
        try:
            for selector in [
                ...
                try:
                    ...
                    if await el.count() > 0 and await el.is_visible():
                        await el.fill(code)
                        logger.info(f"[BiliQR] 已填入验证码到 {selector}")
                        ...
                        logger.info(f"[BiliQR] 会话 {session_id} 验证码已提交")
                        return {'success': True}
                except Exception:
                    continue
    ...
    async def _close_session(self, session_id: str):
        ...
        try:
            ...
            logger.error(f"[BiliQR] 关闭会话失败: {e}")
        finally:
            self.playwright = None
            self.browser = None
    ...
    def get_login_status(self, session_id: str) -> Dict:
        ...
        if status == 'success':
            ...
            has_sessdata = any(c.get('name') == 'SESSDATA' for c in cookies)
            result['has_sessdata'] = has_sessdata
...
bilibili_qr_login_manager = BilibiliQRLogin()
